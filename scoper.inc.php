...
return [
    ...
    'patchers' => [
        function (string $filePath, string $prefix, string $contents): string {
            // Fix `new $variable()` occurrences that require full namespace prefixes.
            // See: https://stackoverflow.com/questions/534159/instantiate-a-class-from-a-variable-in-php
            if (strpos($filePath, 'apereo/phpcas/source/CAS/Client.php') !== false) {
                $prefix_string = '\'\\\\\\\\' . str_replace('\\', '\\\\\\\\', $prefix) . '\\\\\\\\\'';
                // $contents = str_replace(
                //     '$classname = $this->_lang',
                //     '$classname = ' . $prefix_string . ' . $this->_lang',
                //     $contents
                // );
                $contents = preg_replace(
                    '|^(\s+)(.*new )(\$\w+)(\()|m', // Example: `    $something = new $variable(`.
                    "$1$3 = $prefix_string . $3;\n$1$2$3$4",
                    $contents
                );
            }
