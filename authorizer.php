	...
	class WP_Plugin_Authorizer {
		...
		function print_radio_auth_access_who_can_login( $args = '' ) {
			...
			if ( is_array( $args ) && array_key_exists( 'multisite_admin', $args ) && $args['multisite_admin'] === true ) {
				...
				?><div id="overlay-hide-radio_auth_settings_access_who_can_login_external_users" class="auth_multisite_override_overlay"><span class="overlay-note">This setting is overridden by a <a href="<?= network_admin_url( 'admin.php?page=authorizer&tab=access_lists' ); ?>">multisite option</a>.</span></div><?php
			}
		...
		function print_radio_auth_access_who_can_view( $args = '' ) {
			...
			if ( is_array( $args ) && array_key_exists( 'multisite_admin', $args ) && $args['multisite_admin'] === true ) {
				...
				$auth_settings['access_who_can_view'] = $auth_multisite_settings['access_who_can_view'];
				// We're on a site's option page, but there are multisite overrides, so show the overlay.
				?><div id="overlay-hide-radio_auth_settings_access_who_can_view_everyone" class="auth_multisite_override_overlay"><span class="overlay-note">This setting is overridden by a <a href="<?= network_admin_url( 'admin.php?page=authorizer&tab=access_public' ); ?>">multisite option</a>.</span></div><?php
			}
			// Print option elements.
		...
		function print_text_auth_advanced_lockouts( $args = '' ) {
			...
			if ( is_array( $args ) && array_key_exists( 'multisite_admin', $args ) && $args['multisite_admin'] === true ) {
				...
				// We're on a site's option page, but there are multisite overrides, so show the overlay.
				?><div id="overlay-hide-auth_settings_advanced_lockouts_attempts_1" class="auth_multisite_override_overlay"><span class="overlay-note">This setting is overridden by a <a href="<?= network_admin_url( 'admin.php?page=authorizer&tab=advanced' ); ?>">multisite option</a>.</span></div><?php
			}
			// Print option elements.
			?>After
