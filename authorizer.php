...
if ( ! class_exists( 'WP_Plugin_Authorizer' ) ) {
	...
	class WP_Plugin_Authorizer {
		...
		function add_dashboard_widgets() {
			...
			$can_see_dashboard_widget = current_user_can( 'edit_users' );
			if ( is_multisite() ) {
				$can_see_dashboard_widget = current_user_can_for_blog( get_current_blog_id(), 'edit_users' );
			}
			if ( $can_see_dashboard_widget ) {
		...
		function ajax_save_auth_dashboard_widget() {
			// Only users who can see the dashboard widget can save it.
			$can_see_dashboard_widget = current_user_can( 'edit_users' );
			if ( is_multisite() ) {
				$can_see_dashboard_widget = current_user_can_for_blog( get_current_blog_id(), 'edit_users' );
			}
